...
public sealed class UpdateCreditCardCommandValidator : AbstractValidator<UpdateCreditCardCommand>
{
    public UpdateCreditCardCommandValidator()
    {
        ...
        When(x => x.ClosingDay is not null, () =>
            RuleFor(x => x.ClosingDay!.Value).InclusiveBetween((byte)1, (byte)28));

        When(x => x.DueDay is not null, () =>
            RuleFor(x => x.DueDay!.Value).InclusiveBetween((byte)1, (byte)28));

        When(x => x.PaymentAccountId is not null, () =>
            RuleFor(x => x.PaymentAccountId!.Value).NotEmpty());
    }
}
