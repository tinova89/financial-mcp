...
- **Exposed tools** — suggested names, adjust to match the actual implementation:
  ...
  - `list_checking_accounts`/`get_checking_account` (`CheckingAccountTools`, formerly `AccountTools`/`list_accounts`/`get_account`) — **read-only** for financial accounts (checking, investment, wallet, etc.); never returns/operates on credit cards (see below). Create/update/delete are **not** MCP tools — they're REST-only (see below).
  - `list_credit_cards`/`get_credit_card` (`CreditCardTools`) — **read-only** for credit cards. A `CreditCard` is a kind of `Account` (EF Core Table-Per-Hierarchy, same `accounts` table) with its own `ClosingDay`/`DueDay`/`PaymentAccountId`; its `Kind` is always forced to `Credit` and is never a settable parameter. Create/update/delete are also REST-only.
