...
- **Exposed tools** — 18 tools total, all `[McpServerToolType]` classes under `FinancialMcp.Api/Mcp/Tools`, registered by reflection (`WithToolsFromAssembly()` in `Program.cs`). Each is "thin" — see [Mediator Pattern](#mediator-pattern-mediatr):
  ...
  - **`TransactionTools`**
    ...
    - **`remainingBudget`/`remainingBudgetPercentage`** — computed fields on `TransactionDto`, populated only by `create_transaction`, `update_transaction`, and `delete_transaction` (via `ICategoryBudgetRemainingCalculator`, applying the same rules as `get_budget_status`): the post-write budget status for the transaction's parent category/reference month — `null` when no budget goal is in effect. `create_transaction`/`update_transaction` include the transaction's own amount (`includeTransaction: true`); `delete_transaction` excludes it (`includeTransaction: false`), since the transaction no longer counts once soft-deleted. `list_transactions`/`get_transaction` never populate these fields.
  ...
  - **`RevisionTools`** (Card #15) — tools over `transaction_revisions` (the proposed field values submitted while a transaction is in `Status = Revision`).
    - `list_revisions` — lists every pending `transaction_revisions` row, paginated (`page`/`pageSize`) with optional `category`/`accountId` filters, ordered by `createdAt` **ascending** (oldest first). Each item is the `TransactionDto` shape plus `transactionId` and `createdAt`. Read-only; needs only the normal read scope — **no `approval` scope**.
    - `approve_revision` — **moves** (not copies) a revision onto a brand-new `transactions` row with `Status = Scheduled` (stamping `ScheduledAt` now and `SubmittedForReviewAt` copied **verbatim** from the revision's `createdAt` — never regenerated), then deletes the `transaction_revisions` row. The insert + delete commit atomically (`ITransactionalRequest` → `TransactionBehavior`). Params: `revisionId`, `confirm` (must be explicitly `true`, same guard as `delete_transaction`). **Requires the `approval` JWT scope** — a missing scope is rejected with `403` at the tool level, before MediatR (see [Authentication](#authentication-custom-jwt)); it is not a validation error. 404 if `revisionId` is unknown.
- **Enum parameters are sent as plain integers, not strings.** No `JsonStringEnumConverter` is registered, so C#-enum-typed MCP parameters — `create_transaction`'s `type`/`status`/`recurrence`, `update_transaction`'s `status`, `list_transactions`' `type`/`status`, and `create_category_budget`'s `period` — serialize in the tool schema as `int`, not the enum member's name. Each tool's `Description` documents the mapping inline; consult the tool description rather than assuming the string name is accepted. Current mappings (`FinancialMcp.Domain.Enums`): `TransactionStatus` is `1 - Revision`, `2 - Scheduled`, `3 - Confirmed` (Card #14 — see [Budget goals](#budget-goals-get_budget_status)); `TransactionType` is `1 - Expense`, `2 - Income`, `3 - Transfer`, `4 - Payment`; `RecurrenceType` is `0 - None`, `1 - Installment`, `2 - FixedMonthly`; `BudgetPeriodType` is `1 - Monthly`, `2 - OneTime`.
...
- **Scopes** (`scope` claim, one per value; issued by `POST /auth/token` and `/auth/refresh` — see `AuthScopes.Default`): `transactions:read`, `transactions:write`, `budget:read`, and `approval` (Card #15). `approval` gates `approve_revision` only: the tool checks the scope inline and throws a `403`-mapped `ForbiddenException` **before** dispatching to MediatR (MCP tools carry no ASP.NET Core endpoint metadata, so this isn't an `[Authorize]`/validator check). The equivalent endpoint policy — `AuthScopes.ApprovalPolicy` (`"ApprovalScope"`, a `RequireClaim("scope", "approval")`) — is registered for when the global JWT middleware is (re-)enabled. Who *earns* the `approval` scope (role/account policy) is deliberately not decided yet.
...
- **Explicit CQRS:** always separate into **Commands** (writes: `create_transaction`, `update_transaction`, `delete_transaction`, `confirm_transaction`, `approve_revision`, `import_statement`, `create_category_budget`, plus the REST-only account/credit-card create/update/delete) and **Queries** (reads: `list_transactions`, `get_transaction`, `list_revisions`, `list_categories`, `lookup_category`, `get_budget_status`, `list_checking_accounts`, `get_checking_account`, `list_credit_cards`, `get_credit_card`).
...
- Specifically cover with tests:
  ...
  - Card #20 — unit tests for every previously-uncovered tool handler/validator: `list_categories` parent/subcategory grouping, an empty-subcategory-list not erroring, and the DTO carrying no budget-goal information (`ListCategoriesQueryHandlerTests`); `list_transactions` narrowing per filter (`type`/`status`/`accountId`/period/`category`/`subcategory`/`year`/`month`), pagination, `expectedDate`-descending ordering, and `needsConfirmation` — including the known, unfixed quirk that `subcategory`/`year`/`month` run in memory *after* `Skip`/`Take`, so `TotalCount` can exceed `Items.Count` (`ListTransactionsQueryHandlerTests`); `get_transaction`'s full DTO, 404, and soft-delete exclusion via the real query filter (`GetTransactionQueryHandlerTests`, SQLite `:memory:`); `create_transaction`'s per-status timestamp stamping and `remainingBudget`/`remainingBudgetPercentage` pass-through (`CreateTransactionCommandHandlerTests`), plus its previously-uncovered per-statement-type validator rules — `confirmationDate` required for `Confirmed`, `invoiceDueDate`/installment fields required for a credit-card `accountId` (added to the existing `CreateTransactionCommandValidatorTests`); `update_transaction`'s partial-patch semantics, the "stamp once" `TransitionTo` rule proven across two sequential status changes, and post-write budget recalculation with `includeTransaction: true` (`UpdateTransactionCommandHandlerTests`, SQLite `:memory:`); `delete_transaction`'s soft delete, query-filter exclusion, and recalculation with `includeTransaction: false` (`DeleteTransactionCommandHandlerTests`, SQLite `:memory:`), plus its `confirm` guard — which lives only in the validator, not the handler (new `DeleteTransactionCommandValidatorTests`); `list_checking_accounts`/`get_checking_account` TPH exclusion of credit-card rows and `creditCardIds` aggregation (`ListCheckingAccountsQueryHandlerTests`, `GetCheckingAccountQueryHandlerTests`, SQLite `:memory:`); `list_credit_cards`/`get_credit_card` TPH scoping and the always-`Credit` `Kind` (`ListCreditCardsQueryHandlerTests`, `GetCreditCardQueryHandlerTests`, SQLite `:memory:`); `get_budget_status`'s `BudgetGoal.ResolveEffective` precedence (a `Monthly` goal carrying forward until superseded; `OneTime` beating an applicable `Monthly` goal for its own month), the per-account-kind reference month, the `RemainingBudget`/`PercentUsed` formulas (including `PercentUsed = null` when `GoalAmount = 0`), and exclusion of categories with no effective goal (`GetBudgetStatusQueryHandlerTests`, SQLite `:memory:`); `create_category_budget`'s handler-level category-not-found path and its validator-only parent-category/duplicate-goal rejections (`CreateCategoryBudgetCommandHandlerTests`); `import_statement`'s `Account.Kind`-driven routing to the parser, warnings/lines-imported pass-through, and atomic batch commit via `TransactionBehavior` (`ImportStatementCommandHandlerTests`, SQLite `:memory:`), plus the real `StatementCsvParser`'s CSV-format edge cases (`StatementCsvParserTests`, referencing `FinancialMcp.Infrastructure` from the test project) — which surfaced two real gaps between the parser's doc comment and its actual behavior, both left as-is per this card's tests-only scope: a wrong delimiter silently produces a garbage all-default-valued row instead of a warning (missing columns bypass the custom `TypeConverter`s entirely), and once one row throws mid-parse, every row after it is silently dropped too, not just the bad one (the underlying `GetRecords<T>()` iterator doesn't resume after an exception escapes `MoveNext()`) — see `StatementCsvParserTests` for the locked-in current behavior.
